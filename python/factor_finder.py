...
import math,sys
print('''Factor Finder, by Al Sweigart al@inventwithpython

A number's factors are two numbers that, when multiplied with each
other, produce the number. For exampls, 2 x 13 = 26, so 2 and 13 are
...
while True:#Main program loop.
    ...
    response=input('> ')
    if response.upper()=='QUIT':sys.exit()
    if not(response.isdecimal()and int(response)>0):continue
    number=int(response)
    factors=[]
    for i in range(1,int(math.sqrt(number))+1):#Find the factors of number
    ...
    factors.sort()
    #Display the results:
    factors=[str(factor)for i,factor in enumerate(factors)]
    print(', '.join(factors))