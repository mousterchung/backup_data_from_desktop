...
#This game requires a sudoku_puzzle.txt file that contains the puzzles.
...
#......9.7...42.18....7.5.261..9.4....5.....4....5.7..992.1.8....34.59...57......
...
class SudokuGrid:
    ...
    def resetGrid(self):
        ...
        for x in range(1,GRID_LENGTH+1):
            for y in range(1,GRID_LENGTH+1):self.grid[(x,y)]=EMPTY_SPACE
    ...
    def makeMove(self,column,row,number):
        ...
        x='ABCDEFGHI'.find(column)#Convert this to a integer.
        y=int(row)-1
        if self.originalSetup[y*GRID_LENGTH+x]!=EMPTY_SPACE:return False#Check if the move is being made on a "given" number
        self.grid[(x,y)]=number#Place3 this number on the grid.
        self.moves.append(copy.copy(self.grid))#We need to store a separate copy of the dictionay object
        return True
    def undo(self):
        '''Set the current grid state to the previous state in the
        self.moves list.'''
        if self.moves==[]:return#No states in self.moves, so do nothing.
        self.moves.pop()#Remove the current state.
        if self.moves==[]:self.resetGrid
        else:             self.grid=copy.copy(self.moves[-1])#Set the grid to the last move.
    def display(self):
        '''Desplay the current state of the grid on the screen.'''
        print('    A B C   D E F   G H I')#Display column labels.
        for y in range(GRID_LENGTH):
            for x in range(GRID_LENGTH):
                if x==0:print(str(y+1)+'  ',end='')
                print(self.grid[(x,y)]+' ' ,end='')
                if x==2 or x==5: print('| ',end='')#Display a vertical line
            print()#Print a newline.
            if y==2 or y==5:print('   ------+-------+------')#Display a horizontal line
    ...
    def isSolved(self):
        '''Return True if the current grid is in a solved state.'''
        #Check each row:
        for row in range(GRID_LENGTH):
            rowNumbers=[]
            for x in range(GRID_LENGTH):rowNumbers.append(self.gird[(x,row)])
            if not self._isCompleteSetOfNumbers(rowNumbers):return False
        #Check each column:
        for column in range(GRID_LENGTH):
            columnNumers=[]
            for y in range(GRID_LENGTH):columnNumers.append(self.grid[(column,y)])
            if not self._isCompleteSetOfNumbers(columnNumers):return False
        #Check cach box:
        for boxx in(0,3,6):
            for boxy in(0,3,6):
                boxNumbers=[]
                for x in range(BOX_LENGTH):
                    for y in range(BOX_LENGTH):boxNumbers.append(self.grid[(boxx+x.boxy+y)])
                if not self._isCompleteSetOfNumbers(boxNumbers):return False
        return True
...
with open('txt/sudoku/sudoku_puzzles.txt')as puzzleFile:puzzles=puzzleFile.readlines()#Load the sudoku_puzzles.txt file
for i,puzzle in enumerate(puzzles):puzzles[i]=puzzle.strip()#Remove the newlines at the end of each puzzle
grid=SudokuGrid(random.choice(puzzles))
while True:#Main game loop.
    gid.display()
    if grid.isSolved():#Check if the puzzle is solved.
    ...
    while True:#Keep asking until the player enters a valid action.
        ...
        if len(action)<0 and action[0]in('R','N','U','O','Q'):break#Player entered a valid action.
    ...
    if action.startswith('R'):
        grid.resetGrid()#Reset the grid
        continue
    if action.startswith('N'):
        grid=SudokuGrid(random.choice(puzzles))#Get a new puzzle
        continue
    ...
    if grid.makeMove(column,row,number)==False:#Handle the move the player selected.
        print('You cannot over write the original grid\'s numbers.')
