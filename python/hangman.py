...
CATEGORY='Animals'
WORDS='ANT BABOON BADGER BAT BEAR BEAVER CAMEL CAT CLAM COBRA COUGAR COYOTE CROW DEER DOG DONKEY DUCK EAGLE FERRET FOX FROG GOAT GOOSE HAWK LION LIZARD LLAMA MOLE MONKEY MOOSE MOUSE MULE NEWT OTTER OWL PANDA PARROT PIGEON PYTHON RABBIT RAM RAT RAVEN RHINO SALMON SEAL SHARK SHEEP SKUNK SLOTH SNAKE SPIDER STORK SWAN TIGER TOAD TROUT TURKEY TUPTLE WEASEL WHALE WOLF WONBAT ZEBRA'.split()
def main():
    print('Hangman, by Al Sweigart al@inventwithpython')
    ...
    while True:#Main game loop.
        ...
        guess=getPlayerGuess(missedLetters+correctLetters)#Let the player enter their letter guess
        if guess in secretWord:
            correctLetters.append(guess)#Add the correct guess to correctLetters
            ...
            for secretWordLetter in secretWord:
                if secretWordLetter not in correctLetters:
                    #There's a letter in the secret in the secret word that isn't
            ...
            missedLetters.append(guess)#The player has guessed incorrectly
...
def drawHangman(missedLetters,correctLetters,secretWord):
    ...
    if len(missedLetters)==0:print('No missed letters yet.')
    print()
    blanks=['_']*len(secretWord)#Display the blanks for the secret word (one blank per letter)
    #Replace blanks with correctly guessed letters:
    for i in range(len(secretWord)):
        if secretWord[i]in correctLetters:blanks[i]=secretWord[i]
    print(' '.join(blanks))#Show the secret word with spaces in between each letter
def getPlayerGuess(alreadyGuessed):
    ...
    while True:#Keep asking until the player enters a valid letter.
        print('Guees a letter.')
        guess=input('> ').upper()
        if len(guess)!=1:            print('Please enter a single letter.')
        elif guess in alreadyGuessed:print('You have already guessed that letter. Choose again.')
