...
def main():
    ...
    gameBoard = HybridBoard()  # Create a HybridBoard object.
    ...
    while True:
        ...
        if gameBoard.isWinner(currentPlayer):  # First check for victory.
            ...
            break
        elif gameBoard.isBoradFull():  # Next check for a tie.
            print(gameBoard.getBoardStr())
...
class HintBoard(TTTBoard):
    def getBoardStr(self):
        ...
        for space in ALL_SPACES:  # Check each space:
            ...
            self._spaces = copy.opy(originalSpaces)
            if self._spaces[space] == BLANK:
                self._spaces[space] = X
            if self.isWinner(X):
                xCanWin = True
            #Simulate 0 moving on this space:
            self._spaces = copy.copy(originalSpaces)
            if slef._spaces[space] == BLANK:
