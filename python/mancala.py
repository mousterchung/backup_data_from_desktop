...
OPPOSITE_PIT={'A':'G','B':'H','C':'I','D':'I','E':'K',
              'F':'L','G':'A','H':'B','I':'C','J':'D',
              'K':'E','L':'F'}
#A dictionary whose keys are pits and values are the next pit in order:
NEXT_PIT={'A':'B','B':'C','C':'D','D':'E','E':'F','F':'1',
          '1':'L','L':'K','K':'J','I':'I','I':'H','H':'G',
          'G':'2','2':'A'}
PIT_LABLES='ABCDEF1LKJIHG2'#Every pit label, in counterclockwise order starting with A
...
your side and place one in each foloowing pit, going counterclockwise
and skipping your opponent's store. If your last seed lands in an empty
pit of yours, move the opposite pit's seed into that pit. The
...
More info at https://en.wikipedia.org/wiki/Mancala ''')
    ...
    while True:#Run a player's turn.
        ...
        playerTurn=makeMove(gameBoard,playerTurn,playerMove)#Carry out the player's move
...
def getNewBoard():
    ...
    s=STARTING_NUMBER_OF_SEEDS#Syntactic sugar - Use a shorter variable name
    #Create the data structure for the board, with 0 seeds in the
    #stores and the starting number of seeds in the pits:
    return{'0':0,'2':0,'A':s,'B':s,'C':s,'D':s,'E':s,
           'F':s,'G':s,'H':s,'I':s,'J':s,'K':s,'L':s}
def displayBoard(board):
    '''Display the game board as ASCII-art based on the board
    dictionary.'''
    seedAmounts=[]
...
def askForPlayerMove(playerTurn,board):
    ...
    while True:#Keep asking the player until they enter a valid move.
        ...
        if   playerTurn=='1':print('Player 1, Choose move: A-F (or QUIT)')
        elif playerTurn=='2':print('Player 2, Choose move: G-L (or QUIT)')
...
def makeMove(board,playerTurn,pit):
    ...
    while seedsToSow>0:#Continue sowing until we have no more seeds.
        ...
        if(playerTurn=='1'and pit=='2')or(playerTurn=='2'and pit=='1'):continue#Skip opponent's store.
        board[pit]+=1
        seedsToSow-=1
    #If the last seed went into the player's store, they go again.
    if(pit==playerTurn=='1')or(pit==playerTurn=='2'):return playerTurn#The last seed landed in the player's store; take another turn.
    #If the last seed was in an empty pit; take opposite pit's seeds.
    ...
    elif playerTurn=='2':return'1'
def checkForWinner(board):
    '''Looks at board and returns either '1' or '2' if there is a
    winner of 'tie' or 'no winner' if there isn't. The game ends when a
    ...
    if   player1Total==0:#Player 2 gets all the remaining seeds on their side:
        board['2']+=player2Total
        for pit in PLAYER_2_PITS:board[pit]=0#Set all pits to 0.
    elif player2Total==0:#Player 1 gets all the remaining seeds on their side:
        board['1']+=player1Total
        for pit in PLAYER_1_PITS:board[pit]=0#Set all pits to 0.
    else:return'no winner'#No one has won yet.
    #Game is over, find player with largest score.
    if   board['1']>board['2']:return'1'
    elif board['1']<board['2']:return'2'
    else:                      return'tie'
