...
the middle flower space are safe and cannot be landed on.''')
    ...
    while True:#Main game loop.
        ...
        for i in range(4):#Flip 4 coins.
            ...
            if result==0:print('T',end='')#Tails.
            else:        print('H',end='')#Heads.
            if i!=3:     print('-',end='')#Print separator.
            flipTally+=result
        print('  ',end=='')
        if flipTally==0:
            ...
            continue
        validMoves=getValidMoves(gameBoard,turn,flipTally)#Ask the player for their move
        if validMoves==[]:
        ...
        while True:
            ...
            if move=='quit':
                ...
                sys.exit()
            if move in validMoves:break#Exit the loop when a valid move is selected.
        ...
        if movingOntoGoal:
            gameBoard[goal]+=1
            if gameBoard[goal]==7:#Check if the player has won
                displayBoard(gameBoard)
            ...
            if gameBoard[nextBoardSpace]==opponent:gameBoard[opponentHome]+=1#Check if the opponent has a tile there
            gameBoard[nextBoardSpace]=turn#Set the "to" space to the player's token
        ...
        if nextBoardSpace in FLOWER_SPACES:
            ...
            input('Press Enter to continue...')
        else:turn=opponent#Swap turns to the other player.
def getNewBoard():
    ...
    at the home and goal of both player.'''
    board={X_HOME:7,X_GOAL:0,O_HOME:7,O_GOAL:0}
    for spaceLabel in ALL_SPACES:board[spaceLabel]=EMPTY#Set each space as empty to start
    return board
def displayBoard(board):
    ...
    #Add the strings that should populate BOARE_TEMPLATE in order,
    #going from left to right, top to bottom.
    spaces=[]
    spaces.append(xHomeTokens)
    spaces.append(xGoalTokens)
    for spaceLabel in ALL_SPACES:spaces.append(board[spaceLabel])
    spaces.append(oHomeTokens)
    spaces.append(oGoalTokens)
    print(BOARE_TEMPLATE.format(*spaces))
def getValidMoves(board,player,flipTally):
    ...
    if   player==X_PLAYER:
        ...
        track   =O_TRACK
        home    =O_HOME
    if board[home]>0 and board[track[flipTally]]==EMPTY:validMoves.append(home)#Check if the player can move a token from home
    #Check which spaces have a token the player can move:
    for trackSpaceIndex,space in enumerate(track):
        if space=='H'or space=='G'or board[space]!=player:continue
        ...
        if board[nextBoardSpaceKey]in(EMPTY,opponent):
            ...
            if nextBoardSpaceKey=='l'and board['l']==opponent:continue#Skip this move, the space is protected.
            validMoves.append(space)
    return validMoves
