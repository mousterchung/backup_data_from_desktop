...
def askForPlayerMove(board):
    ...
    while True:
        print('                          ({})'.format(w))
        print('Enter WASE (or QUIT): ({}) ({}) ({})'.format(a,s,d))
        response=input('> ').upper()
        if response=='QUIT':sys.exit()
        if response in(w+a+s+d).replace(' ',''):return response
def makeMove(board,move):
    '''Carry out the given move on the given board.'''
    #Note: This function assumes that the move is valid.
    bx,by=findBlankSpace(board)
    if   move=='W':board[bx][by],board[bx  ][by+1]=board[bx  ][by+1],board[bx][by]
    elif move=='A':board[bx][by],board[bx+1][by  ]=board[bx+1][by  ],board[bx][by]
    elif move=='S':board[bx][by],board[bx  ][by-1]=board[bx  ][by-1],board[bx][by]
    elif move=='D':board[bx][by],board[bx-1][by  ]=board[bx-1][by  ],board[bx][by]
...
def getNewPuzzle(moves=200):
    ...
    board=getNewBoard()
    for i in range(moves):makeRandomMove(board)
    return board
