...
#Then save this file as periodic_table.csv.
#Or download this csv file from https://invpy.com/periodctable.csv
import csv,sys,re
#Read in all the data from periodic_table.csv.
elementsFile=open('csv/periodic_table/periodic_table.csv',encoding='utf-8')
...
for key in ALL_COLUMNS:
    if len(key)>LONGEST_COLUMN:LONGEST_COLUMN=len(key)
...
for line in elements:
    ...
    #Some of the data has bracketed text from Wikipedia what we want to
    #remove, such as the atomic weight of Boron:
    #"10.81[III][IV][V][VI]" should be "10.81"
    for key,value in element.items():element[key]=re.sub(r'\[(I|V|X|)+\]','',value)#Remove the [roman numeral] text
...
while True:#Main program loop.
    ...
    print('''            Period Table of Elements
    ...
    if response=='Quit':sys.exit()
    if response in ELEMENTS:#Display the selected element's data:
        for key in ALL_COLUMNS:
