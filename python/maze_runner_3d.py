...
Move around a maze and try to excape... in 3D!
...
def wallStrToWallDict(wallStr):
    ...
    ALL_OPEN or CLOSED) and returns a representation in an dictionary
    wity (x, y) tuples as keys and single-character strings of the
...
#at the start of theis multiline string.
CLOSED={}
...
def displayWallDict(wallDict):
    ...
    for y in range(wallDict['height']):
        print(BLOCK,end='')
        for x in range(wallDict['width']):print(wallDict[(x,y)].replace('.',' '))
...
def pasteWallDict(srcWallDict,dstWallDict,left,top):
    ...
    for x in range(srcWallDict['width']):
        for y in range(srcWallDict['height']):dstWallDict[(x+left,y+top)]=srcWallDict[(x,y)]
    return dstWallDict
def makeWallDict(maze,playerx,playery,playerDirection,exitx,exity):
    ...
    for sec,xOff,yOff in offsets:
        ...
        if(playerx+xOff,playery+yOff)==(exitx,exity):section[sec]=EXIT
        wallDict=copy.copy(ALL_OPEN)
        PASTE_CLOSED_TO={'A':( 6, 4),'B':( 4, 3),'C':( 3, 1),
                         'D':(10, 3),'E':( 0, 0),'F':(12, 0)}
        for sec in'ABDCEF':
            if section[sec]==WALL:wallDict=pasteWallDict(CLOSED[sec],wallDict,*PASTE_CLOSED_TO[sec])
        #Draw the EXIT sign if needed:
        if section['C']==EXIT:wallDict=pasteWallDict(EXIT_DICT,wallDict, 7, 9)
        if section['E']==EXIT:wallDict=pasteWallDict(EXIT_DICT,wallDict, 0,11)
        if section['F']==EXIT:wallDict=pasteWallDict(EXIT_DICT,wallDict,13,11)
        return wallDict
print('Maze Runner 3D, by Al Sweigart al@inventwithpython.com')
print('(Maze files are generated by maze_maker_rec.py)')
while True:#Get the maze file's filename from the user:
    print('Enter the file name of the maze (or LIST or QUIT): ')
    filename=input('> ')
    #List all the maze files in the current folder:
    if filename.upper()=='LIST':
        print('Maze files found in ',os.path.join(os.getcwd(),'txt','maze_runner'))
        for fileInCurrentFolder in os.listdir('txt/maze_runner'):
            if fileInCurrentFolder.startswith('maze')and fileInCurrentFolder.endswith('.txt'):print('  ',fileInCurrentFolder)
        continue
    if filename.upper()=='QUIT':sys.exit()
    if os.path.exists(os.path.join(os.getcwd(),'txt','maze_runner',filename)):break
    print('There is no file named ',filename)
#Load the maze from a file:
mazeFile=open('txt/maze_runner/'+filename)
maze={}
...
for line in lines:
    ...
    for x,character in enumerate(line.rstrip()):
        ...
        if character in(WALL,EMPTY):maze[(x,y)]=character
    ...
    while True:#Get user move.
        ...
        if move=='F'or move=='W':
            ...
            if pDir==WEST  and maze[(px-1,py  )]==EMPTY:
                ...
                break
            elif move.startswith('T'):#Cheat cody: 'T x,y'
                px,py=move.split()[1].split(',')
                px=int(px)
                py=int(py)
                break
            else:print('You cannot move in that direction.')
